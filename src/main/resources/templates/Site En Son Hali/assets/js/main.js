...
function renderCourseCard(course) {
    return `
    <a href="egitim-detay.html?id=${course.id}" class="card course-card" data-aos="fade-up">
...
function renderInstructorCard(instructor) {
    return `
    <div class="card instructor-card" data-aos="fade-up">
...
function initHomePage() {
    ...
    if (grid) {
        ...
        // --- Slider Drag & Auto-Scroll Logic ---
        let isDown = false;
        let startX;
        let scrollLeft;
        let autoScrollTimer;

        // --- Slider Custom Smooth Scroll ---
        const slowScroll = (amount, duration = 1000) => {
            const startPosition = grid.scrollLeft;
            ...
            function animation(currentTime) {
                ...
                // easeInOutQuad function for smooth elegant glide
                let t = timeElapsed / (duration / 2);
                let val;
                if (t < 1) val = (amount / 2) * t * t;
                else {
                    t--;
                    val = - (amount / 2) * (t * (t - 2) - 1);
                }

                grid.scrollLeft = startPosition + val;
                if (timeElapsed < duration) {
                    requestAnimationFrame(animation);
                } else {
                    grid.scrollLeft = targetPosition;
                }
            }
            requestAnimationFrame(animation);
        };

        const slowScrollTo = (target, duration = 1000) => {
            const startPosition = grid.scrollLeft;
            const amount = target - startPosition;
            slowScroll(amount, duration);
        }
            ...
            autoScrollTimer = setInterval(() => {
                if (!isDown) {
                    if (grid.scrollLeft + grid.clientWidth >= grid.scrollWidth - 10) {
                        slowScrollTo(0, 1500); // 1.5 seconds to glide all the way back
                    } else {
                        slowScroll(352, 1000); // Glide left 1 card + gap nicely (320px width + 32px gap)
                    }
                }
            }, 3500); // 3.5 seconds wait between scrolls
        };
        ...
        grid.addEventListener('mousedown', (e) => {
            ...
            stopAutoScroll();
            grid.style.scrollBehavior = 'auto'; // ensure no conflicts
        });
        ...
        // Touch support to reset timer on scroll
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (leftHamburger) leftHamburger.addEventListener('click', toggleDrawer);
...
// ============================================
// ÇEREZ (COOKIE) CONSENT BANNER
// ============================================
(function initCookieBanner() {
    const STORAGE_KEY = 'guzem_cookie_consent';
    const consent = localStorage.getItem(STORAGE_KEY);

    // Daha önce karar verilmişse banner gösterme
    if (consent === 'accepted' || consent === 'declined') {
        if (consent === 'declined') showDeclinedBar();
        return;
    }

    // Banner HTML'i oluştur ve sayfaya ekle
    ...
    banner.innerHTML = `
        <div class="cookie-text">
            ...
            <p>
                <strong>Bu site çerez kullanmaktadır.</strong>
                Deneyiminizi geliştirmek, analiz yapmak ve kişiselleştirilmiş içerik sunmak için çerezler kullanıyoruz.
                <a href="gizlilik.html">Gizlilik Politikası</a> ve <a href="kvkk.html">KVKK Aydınlatma Metni</a>'ni inceleyebilirsiniz.
            </p>
        </div>
        <div class="cookie-actions">
            <button class="cookie-accept" id="cookieAccept">✓ Kabul Et</button>
            <button class="cookie-decline" id="cookieDecline">✕ Reddet</button>
        </div>
    `;

    document.body.appendChild(banner);

    // Yavaşça aşağıdan kaydırarak göster
    ...
    function showDeclinedBar() {
        const bar = document.createElement('div');
        bar.id = 'cookie-declined-bar';
        bar.innerHTML = `🚫 Zorunlu olmayan çerezler reddedildi.
            <span id="cookieReset">Tercihimi değiştir</span>`;
...
(function initScrollToTop() {
    ...
    // 300px aşağı inilince göster
    window.addEventListener('scroll', () => {
        btn.classList.toggle('visible', window.scrollY > 300);
    }, { passive: true });

    // Tıklanınca smooth scroll to top
    btn.addEventListener('click', () => {
